...
object SyncUseCases:
  ...
  /**
   * The limits these use cases enforce, expressed where they are enforced.
   *
   * A domain type rather than a reach into the service's configuration: the rule *"a caller may not wait
   * longer than this"* belongs to the use case, while where the number comes from — a HOCON file, a flag, a
