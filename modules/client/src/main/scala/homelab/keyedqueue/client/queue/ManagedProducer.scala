 ...
 *
 * What it adds over [[QueueClient]] is the envelope: the value names itself, the encoder writes it and
 * says what format it wrote, and the payload type is the one this producer was built for. What it takes
 * away is the depth a send answers with, which is a measure rather than a decision.
 *
 * @param client what the calls are made through
 * @param queue which queue it sends to
 * @param payloadType the schema name and version every message it sends states
...
final private[queue] class ManagedProducer[A: MessageEncoder as encoder](
  ...
  override def emit(value: A): IO[ServiceError, Unit] =
    ...
    client.enqueue(queue, Message.Outgoing(key, id, payloadType, value)).unit
