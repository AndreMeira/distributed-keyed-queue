...
final class RedisLockStore(
    ...
    monitor.trace("RedisLockStore.acquire"):
      Clock.instant.flatMap: asked =>
        readiness.subscribe(acquisition.name).flatMap { mailbox =>
          connection
            .provide:
              scripts.acquire.execute(layout.lock(acquisition.name), acquisition.name, acquisition.ttl, acquisition.patience)
            .flatMap:
  ...
  private def turn(
    ...
    monitor.trace("RedisLockStore.grant"):
      ticket.get.flatMap: id =>
        connection
          .provide(scripts.grant.execute(layout.lock(acquisition.name), acquisition.name, id, acquisition.ttl))
          .flatMap:
  ...
  private def reenter(
    ...
    remainingTime(acquisition.patience, asked).flatMap:
      case None       => ZIO.none
      case Some(left) =>
        connection.provide(scripts.acquire.execute(layout.lock(acquisition.name), acquisition.name, acquisition.ttl, left)).flatMap {
          case AcquireScript.Entered.Granted(token, until)  => ZIO.some(hold(acquisition.name)(token, until))
  ...
  private def withdraw(name: LockName, ticket: Ref[Long]): UIO[Unit] =
    ticket.get.flatMap(id => connection.provide(scripts.abandon.execute(layout.lock(name), name, id)).ignore)
